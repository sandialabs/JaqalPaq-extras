...
def schedule_instr(circ, instr, target, freeze_timestamps, after=-1):
    ...
    if (is_block and instr.parallel) or is_gate:
        ...
        while defrost < len(target) and not can_parallelize(
            circ, target[defrost], instr, used_qubits
        ):
            defrost += 1
...
def can_parallelize(circ, block, instr, qubits):
    if isinstance(block, BlockStatement) and block.parallel:
        ...
        if isinstance(instr, GateStatement):
            return can_parallelize_subinstr(circ, block) and can_parallelize_gate(
                circ, BlockStatement(True, [block]), instr, qubits
            )
        elif isinstance(instr, BlockStatement):
            return can_parallelize_subinstr(circ, block) and can_parallelize_gate(
                circ, instr, block, qubits
            )
        else:
            return False  # We don't know what this is, so we can't parallelize it.
    else:
...
def can_parallelize_subinstr(circ, sub_instr):
    ...
    if sub_instr.name not in circ.native_gates:
        return (
            False  # Can't do macros in parallel, because they could include anything.
        )
    if (
        len(
            [
