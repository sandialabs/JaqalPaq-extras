from jaqalpaq.core import ScheduledCircuit, GateStatement
...
def jaqal_circuit_from_qiskit_circuit(circuit, names=None, native_gates=None):
        ...
        if instr[0].name == "measure":
            ...
            block.append(
                qsc.build_gate(
                    names[instr[0].name],
                    *[
                        qsc.registers[target.register.name][target.index]
                        for target in targets
                    ],
    ...
    if not (
        isinstance(qsc.body[-1], GateStatement) and qsc.body[-1].name == "measure_all"
    ):
        qsc.gate("measure_all")
    return qsc
